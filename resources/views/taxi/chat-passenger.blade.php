...
<div id="messages" class="flex-1 overflow-y-auto mb-3">
    @foreach($messages as $msg)
        <div class="my-1 {{ $msg->sender_type=='passenger'?'text-right':'' }}">
            <span class="px-3 py-2 rounded-lg inline-block 
                {{ $msg->sender_type=='passenger'?'bg-yellow-400':'bg-gray-200' }}">
                {{ $msg->message }}
            </span>
        </div>
    @endforeach
...
.listen('.TaxiMessageSent', (e) => {
    ...
    box.innerHTML += `<div class="my-1"><span class="px-3 py-2 rounded-lg inline-block bg-gray-200">${e.message.message}</span></div>`;
